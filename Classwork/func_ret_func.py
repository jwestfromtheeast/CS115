...
def divides(n):  # What does this do? Call with divides(n)(k)
    def div(k):
        return n % k == 0
    return div
...
' Exercise
'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
def increment(n): # Would call with 
    '''Returns a function that takes in k and adds it to n.'''
    def add_n_to(k):
        return n + k
    return add_n_to

def inc_all(nums, n): # This is what makes it useful. Need to do this to operate with maps
