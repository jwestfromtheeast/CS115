...
def letterScoreLoop(letter, scorelist):
    ...
    for score in scorelist:
        if letter == score[0]:
            return score[1]
    # Even though we assume letter will occur, good practice to always return
    return -1
...
def wordScoreLoop(S, scorelist):
    ...
    scoreSum = 0
    for character in S:
        scoreSum += letterScoreLoop(character, scorelist)
    return scoreSum    
...
def wordsWithScoreLoop(dct, scores):
    ...
    wordAndScore = []
    for word in dct:
        wordAndScore.append([word, wordScoreLoop(word, scores)])
    return wordAndScore
